/**
 * SQL code to create the database.  
 */
/* Entities that need to be changed */
DROP TABLE Skill CASCADE;

DROP TABLE Class CASCADE;

DROP TABLE Charachter CASCADE;

DROP TABLE Gem CASCADE;

DROP TABLE Equipment CASCADE;

DROP TABLE Head_Armour CASCADE;

DROP TABLE Chest_Armour CASCADE;

DROP TABLE Legs_Armour CASCADE;

DROP TABLE Feet_Armour CASCADE;

DROP TABLE Main_Weapon CASCADE;

DROP TABLE Secondary_Equipment CASCADE;

DROP TABLE Head_Armour_Instance CASCADE;

DROP TABLE Chest_Armour_Instance CASCADE;

DROP TABLE Legs_Armour_Instance CASCADE;

DROP TABLE Feet_Armour_Instance CASCADE;

DROP TABLE Main_Weapon_Instance CASCADE;

DROP TABLE Secondary_Equipment_Instance CASCADE;

/* Relationships */
DROP TABLE Earned_Skill CASCADE;

DROP TABLE Auto_Skill CASCADE;

DROP TABLE Has_Earned CASCADE;

DROP TABLE Class_Equipment CASCADE;

DROP TABLE Embedded_Gems CASCADE;

DROP TABLE Clan CASCADE;

DROP TABLE Clan_Member CASCADE;

DROP TABLE head_embed CASCADE;

DROP TABLE chest_embed CASCADE;

DROP TABLE leg_embed CASCADE;

DROP TABLE feet_embed CASCADE;

DROP TABLE main_embed CASCADE;

DROP TABLE secondary_embed CASCADE;

